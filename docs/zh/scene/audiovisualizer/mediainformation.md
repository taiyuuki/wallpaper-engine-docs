...
本指南建立在[集成专辑封面](/wallpaper-engine-docs/scene/audiovisualizer/albumcover)功能之上。虽然在本节中讨论的任何内容中，专辑封面集成不是必需的，但我们仍然建议你先阅读集成专辑封面的指南，然后再继续阅读本指南。虽然本指南建立在集成专辑封面指南之上，但你可以完全独立地使用所有功能。
...
首先，我们需要为要显示的每个元素创建**文本**图层。单击左侧的**添加组件**，然后向壁纸添加新的**文本**图层。

对于具有动态内容的文本图层，你需要记住的一件事是，它们可能会很长。这就是为什么正确配置媒体信息的文本层很重要：
...
5. 最后，你可能希望勾选**溢出省略号**选项。这将截断太长的文本，并将结尾替换为三个点“...”表示文本被切断。
...
创建好每个图层后，可以利用 SceneScript 中的[MediaPropertiesEvent](/wallpaper-engine-docs/scene/scenescript/reference/class/MediaPropertiesEvent)来检索所需的数据。为此，请选择第一个图层（例如歌曲图层），然后单击图层的**文本**属性旁边的齿轮图标并选择**绑定脚本**。你将看到一个示例代码片段，你可以用下面的示例代码替换它。
...
将代码复制到每个相应的文本层并使用编辑器顶部的**运行预览**后，开始使用兼容的媒体播放器播放音乐。你现在应该看到所有对应的数据都将出现，前提是你的媒体播放器将实际数据发送给了Windows。

## 通过SceneScript获取并使用专辑封面颜色

你还可以通过[mediaThumbnailChanged](/wallpaper-engine-docs/scene/scenescript/reference/event/media)事件访问专辑封面颜色，特别是以函数参数形式提供的[MediaThumbnailEvent](/wallpaper-engine-docs/scene/scenescript/reference/class/MediaThumbnailEvent)类的实例。

**在此示例中，我们将展示如何更改整个壁纸的背景颜色以匹配专辑封面。**

::: danger 请注意
在此示例中，我们将更改背景颜色。只有当你的整个壁纸没有背景图像时，这才有意义。

**此处显示的代码适用于编辑器中的任何颜色属性。你还可以将其应用于编辑器中任意文本图层颜色或任意其他颜色属性。**
:::
